...
// App aggregates dependencies and exposes ports for the adapters (HTTP, workers, etc.).
...
// To use plugins: inject implementations of ports (e.g. pkg/database/postgres as Repository).
func New() *App {
...
// Option configures the application (e.g. replace Repository with a DB client).
...
// RegisterCloser registers a callback for shutdown (e.g. close plugin connections).
...
// Shutdown executes all registered closers (plugins and adapters).
